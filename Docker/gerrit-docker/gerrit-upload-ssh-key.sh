...
stored_key=$(echo -e $(curl --user ${username}:${password} --silent http://localhost:8080//a/accounts/${user}/sshkeys | grep "ssh_public_key" | awk '{split($0, a, ": "); print a[2]}' | sed 's/[",]//g'))
...
ret=$(curl --request POST --user "${username}:${password}" --data "${ssh_key}" --output /dev/null --silent --write-out "%{http_code}" "http://localhost:8080/a/accounts/${user}/sshkeys")
if [[ ${ret} -eq 201 ]]; then
