...
ret=$(curl --user "${admin_user}:${admin_password}" --output /dev/null --silent --write-out "%{http_code}" "http://localhost:8080/a/accounts/${username}")
if [[ ${ret} -eq 404 ]] ; then
...
ret=$(curl --user "${admin_user}:${admin_password}" --output /dev/null --silent --write-out "%{http_code}" "http://localhost:8080/a/groups/${target_group}")
if [[ ${ret} -eq 404 ]] ; then
...
do
  ...
  ret=$(curl --request POST --user "${admin_user}:${admin_password}" --header 'Content-Type: application/json; charset=UTF-8' --data "${json_request}" --output /dev/null --silent --write-out "%{http_code}" http://localhost:8080/a/groups/"${target_group}"/members.add)
  if [[ ${ret} -eq 200 ]]; then
